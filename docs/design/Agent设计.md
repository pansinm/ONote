# Agent 功能设计文档

## 概述

Agent 功能为 ONote 提供智能任务执行能力，通过 LLM 和工具系统实现自动化的文件操作、代码修改、文档生成等复杂任务。Agent 可以自主决策、多轮迭代，并通过 bidc 与主窗口通信完成文件操作。

## 设计目标

- 🎯 **自主执行** - Agent 可以理解用户意图并自动执行复杂任务
- 🔧 **工具系统** - 提供丰富的内置工具（文件操作、搜索等）
- 🔄 **多轮迭代** - 支持多轮对话和工具调用，逐步完成任务
- 📊 **可视化反馈** - 实时显示执行过程、工具调用和结果
- 🚀 **高性能** - 完全在 iframe 内部运行，不阻塞主线程
- 🔒 **安全性** - 通过权限控制和用户确认机制保护用户数据

## 系统架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           主窗口 (Main Window)                          │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  LLMBoxFrame (父窗口)                                           │  │
│  │  - 管理 iframe 通信 (bidc)                                        │  │
│  │  - 响应文件操作请求                                              │  │
│  │  - 通过 onote.dataSource 执行文件操作                               │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                              │                                        │
│                              │ bidc 通信                               │
│                              ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  <iframe> llmbox (子窗口)                                       │  │
│  │  ┌────────────────────────────────────────────────────────────────┐  │  │
│  │  │  UI 层                                                  │  │  │
│  │  │  ├── Chat Mode (现有)                                   │  │  │
│  │  │  └── Agent Mode (新增)                                  │  │  │
│  │  │      ├── AgentPanel (工具列表 + 执行日志)                   │  │  │
│  │  │      └── AgentInputArea (Agent 输入)                       │  │  │
│  │  └────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌────────────────────────────────────────────────────────────────┐  │  │
│  │  │  状态管理层 (MobX)                                        │  │  │
│  │  │  ├── LLMChatStore (扩展 - 添加 Agent 能力)                   │  │  │
│  │  │  └── AgentStore (新增)                                    │  │  │
│  │  └────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌────────────────────────────────────────────────────────────────┐  │  │
│  │  │  Agent 执行引擎 (新增)                                    │  │  │
│  │  │  ├── ToolRegistry (工具注册表)                              │  │  │
│  │  │  ├── AgentOrchestrator (Agent 编排器)                        │  │  │
│  │  │  └── BuiltInTools (内置工具)                               │  │  │
│  │  └────────────────────────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 核心组件

#### 1. ToolRegistry (工具注册表)

**职责**:
- 管理所有可用工具
- 提供 OpenAI 格式的工具 Schema
- 执行工具调用（通过 bidc 与主窗口通信）

**关键方法**:
```typescript
class ToolRegistry {
  register(tool: Tool): void;
  getAll(): Tool[];
  get(name: string): Tool | undefined;
  getOpenAISchema(): any[];
}
```

#### 2. AgentOrchestrator (Agent 编排器)

**职责**:
- 管理 Agent 执行流程
- 与 LLM 交互
- 多轮迭代和工具调用
- 执行步骤管理

**关键方法**:
```typescript
class AgentOrchestrator {
  run(prompt: string): Promise<void>;
  stop(): void;
  private callLLM(messages: any[]): Promise<any>;
  private executeToolCalls(toolCalls: ToolCall[]): Promise<any[]>;
  private buildSystemPrompt(): string;
}
```

#### 3. AgentStore (Agent 状态管理)

**职责**:
- 管理 Agent 状态
- 工具列表管理
- 执行日志存储
- 与 UI 组件集成

**状态**:
```typescript
interface AgentState {
  tools: Tool[];
  executionLog: ExecutionStep[];
  agentState: 'idle' | 'thinking' | 'executing';
  isRunning: boolean;
  error: string | null;
}
```

#### 4. AgentPanel (UI 组件)

**职责**:
- 显示可用工具列表
- 展示执行日志
- 状态指示
- 用户输入处理

## 工具系统

### 内置工具

| 工具名称 | 描述 | 权限 | 危险 |
|---------|------|-------|-------|
| `readFile` | 读取文件内容 | read | ❌ |
| `writeFile` | 写入文件（覆盖） | write | ✅ |
| `createFile` | 创建新文件 | write | ✅ |
| `deleteFile` | 删除文件 | write | ✅ |
| `listFiles` | 列出目录内容 | read | ❌ |
| `searchFiles` | 搜索文件名 | read | ❌ |
| `searchInFile` | 文件内搜索 | read | ❌ |

### 工具定义格式

```typescript
interface Tool {
  name: string;
  description: string;
  parameters: {
    type: 'object';
    properties: Record<string, ToolParameter>;
    required?: string[];
  };
  executor: (params: any) => Promise<any>;
  metadata?: {
    category: 'file' | 'search' | 'custom';
    permission: 'read' | 'write';
    dangerous?: boolean;
  };
}
```

### 工具执行流程

```
1. LLM 返回工具调用
   ↓
2. AgentOrchestrator 解析 tool_calls
   ↓
3. 从 ToolRegistry 获取工具
   ↓
4. 调用 tool.executor(params)
   ↓
5. ToolExecutor 通过 bidc 发送请求到主窗口
   ↓
6. 主窗口通过 onote.dataSource 执行文件操作
   ↓
7. 返回结果到 ToolExecutor
   ↓
8. 将结果添加到消息历史
   ↓
9. LLM 基于结果决定下一步行动
```

## bidc 通信协议

### 消息类型

```typescript
// 主窗口 → 子窗口
const LLM_BOX_MESSAGE_TYPES = {
  // Agent 文件操作
  AGENT_FILE_READ: 'AGENT_FILE_READ',
  AGENT_FILE_WRITE: 'AGENT_FILE_WRITE',
  AGENT_FILE_CREATE: 'AGENT_FILE_CREATE',
  AGENT_FILE_DELETE: 'AGENT_FILE_DELETE',
  AGENT_FILE_LIST: 'AGENT_FILE_LIST',
  AGENT_FILE_SEARCH: 'AGENT_FILE_SEARCH',
  AGENT_FILE_SEARCH_IN: 'AGENT_FILE_SEARCH_IN',
  
  // Agent 状态查询
  AGENT_GET_ROOT_URI: 'AGENT_GET_ROOT_URI',
  AGENT_GET_ACTIVE_FILE_URI: 'AGENT_GET_ACTIVE_FILE_URI',
} as const;
```

### 通信示例

#### 读取文件

```typescript
// 子窗口发送请求
const response = await channel.send({
  type: 'AGENT_FILE_READ',
  data: { uri: 'file:///path/to/file.md' }
});

// 主窗口处理
receive(async ({ type, data }) => {
  if (type === 'AGENT_FILE_READ') {
    const { uri } = data;
    const content = await onote.dataSource.invoke('readText', uri);
    return { content };
  }
});
```

#### 写入文件

```typescript
// 子窗口发送请求
const response = await channel.send({
  type: 'AGENT_FILE_WRITE',
  data: { uri: 'file:///path/to/file.md', content: '...' }
});

// 主窗口处理
receive(async ({ type, data }) => {
  if (type === 'AGENT_FILE_WRITE') {
    const { uri, content } = data;
    await onote.dataSource.invoke('writeText', uri, content);
    return { success: true };
  }
});
```

## Agent 执行流程

### 执行流程图

```
用户输入任务
   ↓
AgentOrchestrator.run(prompt)
   ↓
构建初始消息 (system + user)
   ↓
开始迭代循环
   ↓
   ├─ callLLM(messages)
   │   ↓
   │   解析 LLM 响应
   │   ↓
   │   └─ 有 tool_calls?
   │       ├─ Yes → 执行工具
   │       │   ↓
   │       │   executeToolCalls(toolCalls)
   │       │   ↓
   │       │   将结果添加到 messages
   │       │   ↓
   │       │   继续下一次迭代
   │       │
   │       └─ No → 完成任务
   │           ↓
   │           显示 final_answer
   │
   └─ 检查是否应该继续 (迭代次数、错误等)
       ↓
   达到最大迭代或完成任务 → 结束
```

### 执行步骤类型

| 步骤类型 | 描述 | 显示内容 |
|---------|------|---------|
| `thinking` | Agent 思考过程 | LLM 返回的文本内容 |
| `tool_call` | 工具调用请求 | 工具名称和参数 |
| `tool_result` | 工具执行结果 | 返回结果或错误信息 |
| `final_answer` | 最终答案 | 任务完成说明 |
| `error` | 错误信息 | 错误详情 |

## 状态管理

### AgentStore 状态

```typescript
class AgentStore {
  // 工具列表
  tools: Tool[];

  // 执行日志
  executionLog: ExecutionStep[];

  // Agent 状态
  agentState: 'idle' | 'thinking' | 'executing';

  // 是否正在运行
  isRunning: boolean;

  // 错误信息
  error: string | null;
}
```

### 状态转换图

```
idle (空闲)
   ↓ 用户输入
thinking (思考中)
   ↓ LLM 返回 tool_calls
executing (执行工具)
   ↓ 工具执行完成
thinking (思考中) ← 或
   ↓ 决定继续执行
executing (执行工具)
   ↓ 任务完成 / 达到最大迭代
idle (空闲)
   ↓ 发生错误
error (错误)
   ↓
idle (空闲)
```

## 文件组织

```
packages/
├── renderer/src/
│   ├── llmbox/
│   │   ├── agent/                          # Agent 模块
│   │   │   ├── types.ts                   # 类型定义
│   │   │   ├── ToolRegistry.ts            # 工具注册表
│   │   │   ├── AgentOrchestrator.ts      # Agent 编排器
│   │   │   └── index.ts                  # 导出
│   │   │
│   │   ├── AgentStore.ts                  # Agent 状态管理
│   │   ├── AgentPanel.tsx                 # Agent UI 组件
│   │   ├── AgentPanel.module.scss         # Agent 样式
│   │   ├── LLMChatStore.ts               # Chat 状态管理（保持不变）
│   │   └── LLMBox.tsx                   # Chat UI（保持不变）
│   │
│   ├── main/containers/LLMBox/
│   │   ├── LLMBoxFrame.tsx               # 父窗口容器
│   │   └── constants.ts                 # 消息类型定义
│   │
│   └── entry/
│       └── llmbox.tsx                    # 子窗口入口
│
└── electron/src/
    └── (无需修改，通过现有的 DataSource 处理)
```

## 使用示例

### 示例 1: 读取文件并总结

```
用户输入:
"读取 /project/README.md 文件的内容并总结"

Agent 执行:
1. [thinking] 我需要先读取 README.md 文件
2. [tool_call] readFile(uri='file:///project/README.md')
3. [tool_result] 返回文件内容
4. [final_answer] 总结文件内容
```

### 示例 2: 批量修改文件

```
用户输入:
"在 /docs 目录下所有 Markdown 文件开头添加版权声明"

Agent 执行:
1. [thinking] 我需要先找到所有 Markdown 文件
2. [tool_call] listFiles(uri='file:///docs')
3. [tool_result] 返回文件列表
4. [thinking] 过滤出 .md 文件
5. [tool_call] readFile(uri='file:///docs/file1.md')
6. [tool_result] 返回文件内容
7. [thinking] 添加版权声明
8. [tool_call] writeFile(uri='file:///docs/file1.md', content='...')
9. [tool_result] 写入成功
10. [thinking] 处理下一个文件...
11. [final_answer] 已完成所有文件修改
```

### 示例 3: 代码重构

```
用户输入:
"将 utils.ts 中的所有函数添加 JSDoc 注释"

Agent 执行:
1. [thinking] 读取文件内容
2. [tool_call] readFile(uri='file:///src/utils.ts')
3. [tool_result] 返回代码
4. [thinking] 分析函数并生成注释
5. [tool_call] writeFile(uri='file:///src/utils.ts', content='带注释的代码')
6. [final_answer] 重构完成
```

## 配置选项

### Agent 配置

```typescript
interface AgentConfig {
  apiKey: string;              // OpenAI API Key
  model: string;               // 模型名称 (gpt-3.5-turbo, gpt-4, etc.)
  apiBase: string;             // API 端点
  fileUri?: string;            // 当前文件 URI
  rootUri?: string;            // 根目录 URI
  maxIterations?: number;       // 最大迭代次数 (默认 10)
  showThinking?: boolean;       // 显示思考过程 (默认 true)
  timeout?: number;            // 超时时间 (毫秒，默认 60000)
}
```

### 执行限制

```typescript
const DEFAULT_LIMITS = {
  maxIterations: 10,          // 最大迭代次数
  maxToolCalls: 50,           // 最大工具调用次数
  maxExecutionTime: 300000,    // 最大执行时间 (5 分钟)
  maxFileSize: 10485760,      // 最大文件大小 (10 MB)
};
```

## 安全考虑

### 权限控制

- **读取工具** (readFile, listFiles, searchFiles) - 无需确认
- **写入工具** (writeFile, createFile) - 需要用户确认
- **危险工具** (deleteFile) - 需要明确用户确认

### 执行限制

- 限制最大迭代次数（默认 10 次）
- 限制最大执行时间（默认 5 分钟）
- 限制单次文件操作大小（默认 10 MB）
- 记录所有工具调用日志

### 用户确认流程

```typescript
interface ConfirmationRequest {
  toolName: string;
  params: any;
  riskLevel: 'low' | 'medium' | 'high';
  reason: string;
}

interface ConfirmationResponse {
  approved: boolean;
  comment?: string;
}
```

## 性能优化

### 1. 工具执行优化

- 并行执行独立的工具调用
- 超时控制机制
- 错误重试策略

### 2. 日志管理

- 限制最大日志条数（默认 100 条）
- 压缩旧的执行步骤
- 虚拟滚动（大量日志时）

### 3. 状态更新优化

- 使用 MobX 的 `runInAction` 批量更新
- 防抖频繁的状态变化
- 避免不必要的 UI 重渲染

## 扩展性

### 自定义工具

未来支持通过插件系统注册自定义工具：

```typescript
// 插件示例
const customTool: Tool = {
  name: 'webSearch',
  description: 'Search the web',
  parameters: {
    type: 'object',
    properties: {
      query: { type: 'string', description: 'Search query' }
    },
    required: ['query'],
  },
  executor: async (params) => {
    const response = await fetch(`https://api.example.com/search?q=${params.query}`);
    return await response.json();
  },
};

toolRegistry.register(customTool);
```

### LLM 提供商扩展

支持多个 LLM 提供商（OpenAI、Anthropic、Gemini）：

```typescript
interface LLMProvider {
  call(messages: any[], tools: any[]): Promise<any>;
  parseResponse(response: any): any;
}
```

## 测试策略

### 单元测试

- ToolRegistry 测试
- AgentOrchestrator 测试
- AgentStore 测试

### 集成测试

- bidc 通信测试
- 文件操作测试
- 端到端工作流测试

### E2E 测试

- 完整任务执行测试
- 多轮迭代测试
- 错误处理测试

## 实现计划

### Phase 1: 基础设施 (Day 1-2)
- [x] 创建类型定义 (`agent/types.ts`)
- [x] 创建 ToolRegistry (`agent/ToolRegistry.ts`)
- [x] 扩展消息类型 (`constants.ts`)
- [x] 修改 LLMBoxFrame 添加文件操作处理

### Phase 2: Agent 核心逻辑 (Day 3-4)
- [ ] 创建 AgentOrchestrator (`agent/AgentOrchestrator.ts`)
- [ ] 创建 AgentStore (`AgentStore.ts`)
- [ ] 实现 LLM 调用和工具执行
- [ ] 实现多轮迭代逻辑

### Phase 3: UI 组件 (Day 5-6)
- [ ] 创建 AgentPanel (`AgentPanel.tsx`)
- [ ] 创建 AgentPanel 样式 (`AgentPanel.module.scss`)
- [ ] 修改 llmbox.tsx 集成 Agent 模式
- [ ] 实现模式切换

### Phase 4: 测试与优化 (Day 7)
- [ ] 端到端测试
- [ ] 错误处理完善
- [ ] 性能优化
- [ ] 用户体验改进

## 相关文档

- [LLMBox 架构说明](../technical/LLMBox架构说明.md) - AI 对话功能架构
- [LLM 对话持久化实现](../technical/LLM对话持久化实现.md) - 对话历史持久化
- [开发指南](../guides/开发指南.md) - 开发流程和最佳实践

---

**文档版本**: 1.0
**创建日期**: 2026-01-06
**最后更新**: 2026-01-06
**作者**: ONote 开发团队
